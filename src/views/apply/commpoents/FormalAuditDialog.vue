  ...
  >
    <el-form ref="form" label-position="right" label-width="80px" :model="form" :rules="rules">
      <el-form-item label="状态" prop="status">
        <el-select v-model="form.status" :placeholder="statusPlaceholder">
          <el-option
            v-for="item in formalAuditStatus"
            :key="item.itemValue"
            :label="item.itemText"
...
export default {
  ...
  computed: {
    rules() {
      return {
        status: [{ required: true, message: this.subjectStatusPlaceholder, trigger: 'blur' }],
    ...
    formalAuditStatus() {
      return this.dataDictionarty_code['formalAuditStatus'].items;
    },
  },
  watch: {
    formData: {
