  ...
  <div class="app-container">
    ...
    <div class="ad-app-table">
      ...
      >
        ...
        <!-- <el-table-column label="医学审查状态" prop="enrollmentStatus" min-width="100">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" min-width="180">
          <template slot-scope="scope">
            <el-button type="text" @click="handleStatusClick(scope.row)">标记形审状态</el-button>
            <el-button type="text" @click="handleSubjectEditClick(scope.row)">编辑受试者信息</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <apply-formal-audit-dialog
      :dialog.sync="dialog"
      :title="title"
...
import ApplyFormalAuditDialog from './commpoents/FormalAuditDialog';
export default {
  name: 'ApplyFormal',
  components: {
    ApplyFormalAuditDialog,
  },
  filters: {},
  mixins: [page, search],
  props: {
  ...
  methods: {
    ...
    managementList() {
      this.loading = true;
      this.$post(this.urls.subject_study.formal, { keyword: this.keyword, ...this.pagination })
        .then(({ data, total }) => {
          this.tableData = data;
          this.total = total;
          this.loading = false;
    ...
    handleSubmitDialogClick(paramet) {
      const { status, remark, id, studyId, subjectId } = paramet;
      const url = this.urls.subject_study[this.dialogType === 'create' ? 'formal_status' : ''];
      const message = this.dialogType === 'create' ? '标记成功' : '';
      this.$post(url, { status, remark, id, studyId, subjectId })
        .then(() => {
          this.managementList();
